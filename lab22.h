...
Equipment::Equipment(int ehp,int eatk,int edef){
    hpmax =ehp;
    atk = eatk;
    def = edef;
}
...
void Unit::equip(Equipment* d){
    if(equipment!=NULL){
        vector<int> wear = equipment->getStat();
        hpmax -= wear[0];
        if(hp > hpmax){
            hp = hpmax;
            }
...
int Unit::ultimateAttack(Unit& type){
	return type.beAttacked(atk*2);
}
